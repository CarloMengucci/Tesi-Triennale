...
Nello specifico la trattazione riguarda il problema dell'elaborazione di risultati consistenti in presenza di dati troncati, fenomeno che interessa anche il database prodotto da Mark-Age a causa del criterio di selezione degli individui campione, prelevati in uno span di età che va dai 35 ai 75 anni.
...
Come emerge dalla trattazione fin'ora svolta, i metodi lineari costituiscono un potente strumento d'indagine predittiva soprattutto grazie alla loro relativa semplicità operativa.

Tuttavia vi sono casi in cui le assunzioni strutturali proprie di questi modelli non sussistono, i risultati forniti risentono perciò fortemente di effetti dovuti ad una scarsa, o spesso nulla, giustificazione analitica nel loro utilizzo.

In presenza di una troncatura deterministica, come nel caso del database sviluppato all'interno del progetto Mark-Age, i canonici metodi basati sui minimi quadrati ordinari perdono di validità, generando ripercussioni sulle performance predittive degli algoritmi e dunque sulla stima dei parametri associati alle variabili.

E' stato infatti possibile evidenziare oltre allo spostamento delle rette di regressione per datasets troncati, dovuto principalmente a fenomeni di leverage relativi al comportamento intrinseco dei minimi quadrati, una forte inferenza anche per i risultati relativi all'ambito di modelling.
...
Analizzando poi le capacità predittive sulla stima dei coefficienti, risulta immediatamente chiaro come esse siano fortemente influenzate dalla mancanza di dati sensibili.

Questi risultati costituiscono i punti necessari per proporre un approccio a situazioni in cui è presente un troncatura in grado di migliorare la qualità generale delle performances di previsione.

Il caso del progetto Mark-Age, che si propone di elaborare dei modelli per l'individuazione della corretta età biologica a partire dall'analisi di un esteso set di parametri fisiologici e biochimici propri dell'essere umano, è un esempio eclatante di presenza di datasets troncati deterministicamente.
...
Deriva da ciò non solo l'impossibilità di proporre come generali i risultati ottenuti, in quanto non validi per una percentuale consistente di osservazioni che si otterrebbero prendendo in considerazioni campioni con età al di sotto e al di sopra dei limiti di età delle popolazioni esaminate, ma anche la difficoltà a proporre dei valori discreti come soluzioni per i parametri.

Con l'introduzione invece di un metodo che tenga conto dell'effetto di troncatura, si ottengono risultati estremamente più consistenti in termini di tendenza e stima dei range probabilistici per i parametri da individuare.

L'individuazione di modelli analiticamente corretti è pertanto necessaria per l'elaborazione di risultati dotati di maggior plausibilità, è però fondamentale mantenere per quanto possibile una certa semplicità operativa.
