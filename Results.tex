...
\subsection{Maximum Likelihood per Distribuzioni Normali Troncate}
...
In generale è infatti presente un campionamento $x_1,x_3,..,x_n$ di $n$ osservazioni indipendenti ed identicamente distribuite, provenienti da una distribuzione la cui \textit{funzione di densità di probabilità }  $f_0(.)$ non è nota. 
E' comunque assunto che essa appartenga ad una certa famiglia di distribuzioni $ \{f(.|\theta),\theta\in\Theta\} $, dove $\theta$ è un vettore di parametri per la famiglia stessa detta \textit{modello parametrico}, cosicché $f_0=f(.|\theta_0)$. 
...
E' ora necessario un approccio specifico al calcolo dell'estimatore di massima verosimiglianza nel caso in cui il campionamento segua un andamento equiparabile a quello di una Gaussiana troncata.
